...
- **Zero bare `<table>` tags** — every table is wrapped in `<div class="w-embed">`. Grep: `html.count('<table>') == html.count('<div class="w-embed"')` for table-only embeds (adjust if you have non-table embeds like CTA blocks).
...
See `references/markdown-conversion.md` for the full conversion ruleset and edge cases, and `references/embed-blocks.md` for embed templates (tables, CTAs, future patterns).
...
### Step 11 — Copy drafts/ → final/

After Step 10 completes green (validation passes + rollout-log row written + Linear ticket closed), copy the entire drafts folder into the matching `final/` folder next to it. The draft folder stays as an historical record — this is a copy, not a move.

Path pairs (same convention as `aeo-content` uses):

| Channel | drafts source | final destination |
|---|---|---|
| Blog post | `content/blog/drafts/{slug}/` | `content/blog/final/{slug}/` |
| Service page | `content/services/drafts/{slug}/` | `content/services/final/{slug}/` |
| Case study | `content/case studies/drafts/{slug}/` | `content/case studies/final/{slug}/` |
| LinkedIn post | `content/linkedin/drafts/{YYYY-MM-DD}-{slug}/` | `content/linkedin/final/{YYYY-MM-DD}-{slug}/` |
| External article | `content/link building/drafts/{host}-{slug}/` | `content/link building/final/{host}-{slug}/` |
| Substack | `content/substack/drafts/{slug}/` | `content/substack/final/{slug}/` |
| Other | `content/other/drafts/{slug}/` | `content/other/final/{slug}/` |
...
drafts = Path(source_dir)                    # ...content/{channel}/drafts/{slug}/
final = drafts.parent.parent / 'final' / drafts.name  # ...content/{channel}/final/{slug}/

if final.exists():
    # Second publish (e.g. re-publish after edit) — versioned final copy
    ts = datetime.now().strftime('%Y%m%d-%H%M%S')
    final = final.with_name(f"{final.name}--republished-{ts}")

shutil.copytree(drafts, final)
```

Rules:

- **Copy, don't move.** `drafts/{slug}/` continues to exist as historical WIP. Editing continues in `drafts/`; `final/` is a snapshot of what actually shipped.
- **If `final/{slug}/` already exists** (second publish of the same post), suffix the new copy with `--republished-{timestamp}` — do not overwrite the previous final. This preserves audit trail across re-publishes.
- **Copy contents only, not the folder itself.** All files inside `drafts/{slug}/` (draft.md, publish-kit.md, featured/thumbnail PNGs, brief.md, etc.) go into `final/{slug}/`.
- **After copy** — update the Linear ticket with a comment noting the final folder location. Then tick the Stage 8a checklist item.
